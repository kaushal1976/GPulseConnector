...
public class BrainboxConnectionHandler : IDisposable
{
    ...
    {
        var ok = await TryConnectAsync(token);
    ...
    private async Task<bool> TryConnectAsync(CancellationToken token)
    {
        try
        {
            await Task.Run(() =>
            {
                ...
                {
                    _logger.LogWarning("Device @{Ip} connection failed: {Message}", _ip, se.Message);
                    IsConnected = false;
                    IsAvailable = false;
                }
                catch
    ...
    private async Task ReconnectLoopAsync(CancellationToken token)
    {
        while (!token.IsCancellationRequested)
        {
            if (!IsConnected)
            {
                ...
                try
                {
                    bool ok = await TryConnectAsync(token);
                if (ok)
    ...
    private void OnDeviceStatusChanged(IDevice<IConnection, IIOProtocol> dev, string property, bool newValue)
    {
        ...
        // ✔ Automatically start reconnect loop on disconnection
        if (!IsConnected)
            EnsureReconnectLoopRunning();
    }
